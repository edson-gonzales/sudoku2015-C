...
class XMLHandler(FileHandler):
    ...
    def retrieve_dict_options_per_tag(self, xml_tag):
        """This method returns a dictionary of options that will be used for Settings Module
        Keyword arguments:
        xml_tag -- tag name (i.e. level, algorithm) from which the search will be narrowed down.
        Returned parameters:
        options -- dictionary of all xml tags with a certain attribute name.
        """
        options = OrderedDict()
        counter = 1
        for element in self.xml_root.iter(xml_tag):
            options[str(counter)] = element.attrib["name"]
            counter = counter + 1
        return options
