...
class SudokuGrid(object):
    ...
    def __init__(self):
        ...
        self.units = dict((square, [u for u in self.unitlist if square in u]) for square in self.squares)
    ...
    def display_2D_grid(self):
        """Display the puzzle values as a 2-D grid. well formatted with - and |."""
        outcome = ""
        width = 1+max(len(self.grid_values[square]) for square in self.squares)
        line = '+'.join(['-'*(width*3)]*3)
        for r in self.rows:
    ...
    def display_2D_grid_with_points(self):
        """Display the puzzle values as a 2-D grid. with points as empty values."""
        outcome = ""
        self.string_grid = self.string_grid.replace('0', '.')
        self.load_grid_values(self.string_grid) 
        outcome = self.display_2D_grid()
        return outcome
