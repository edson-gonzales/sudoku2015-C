...
class TestXMLHandler(unittest.TestCase):
    ...
    def test_default_active_setting_for_algorithms_is_properly_saved(self):
        ...
        self.assertEquals("false", xml.read_active_value_for_setting("algorithm", "Peter Norvig"))
        self.assertEquals("false", xml.read_active_value_for_setting("algorithm", "Brute Force"))

        xml.define_default_active_setting("algorithm", "Peter Norvig")
        xml.save_file()
        self.assertEquals("false", xml.read_active_value_for_setting("algorithm", "Backtracking"))
        self.assertEquals("true", xml.read_active_value_for_setting("algorithm", "Peter Norvig"))
        self.assertEquals("false", xml.read_active_value_for_setting("algorithm", "Brute Force"))

    def test_range_of_missing_numbers_per_level(self):
        xml = XMLHandler()
        xml.load_file()
        xml.change_text_fields("level", "Custom", "min", "40")
        xml.save_file()
        self.assertEquals(str(40), xml.retrieve_text_node_value("level", "Custom", "min"))
        xml.change_text_fields("level", "Custom", "max", "45")
        xml.save_file()
        self.assertEquals(str(45), xml.retrieve_text_node_value("level", "Custom", "max"))

    def test_output_path_changes(self):
        xml = XMLHandler()
        xml.load_file()
        expected = "content/user/"
        xml.change_text_fields("output", "Game Output", "path", expected)
        xml.save_file()
        self.assertEquals(expected, xml.retrieve_text_node_value("output", "Game Output", "path"))
